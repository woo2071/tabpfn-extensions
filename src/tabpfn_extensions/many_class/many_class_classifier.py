...
class ManyClassClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _get_n_estimators(self, n_classes: int, alphabet_size: int) -> int:
        ...
        min_needed_for_potential_coverage = math.ceil(n_classes / max(1, alphabet_size - 1))
        return max(min_estimators_theory, min_needed_for_potential_coverage) * self.n_estimators_redundancy
    ...
    def _generate_codebook(
        ...
        if n_classes <= alphabet_size:
             raise ValueError("_generate_codebook called when n_classes <= alphabet_size")
        ...
        for i in range(n_estimators):
            ...
            noisy_counts = coverage_count + random_state_instance.uniform(0, 0.1, n_classes)
    ...
    def predict_proba(self, X) -> np.ndarray:
        ...
        if self.X_train is None or self.Y_train_per_estimator is None or self.code_book_ is None:
